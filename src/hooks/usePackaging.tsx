...
export function usePackaging() {
  ...
  const createPackaging = useMutation({
    mutationFn: async (packaging: PackagingFormData) => {
      ...
      const { data, error } = await supabase.from('packaging').insert({
        user_id: userId, name: packaging.name, purchase_price: packaging.purchase_price,
        package_quantity: packaging.package_quantity, unit: packaging.unit,
        category_id: packaging.category_id || null, brand: packaging.brand || null,
        supplier: packaging.supplier || null, dimensions: packaging.dimensions || null,
        stock_quantity: packaging.stock_quantity ?? null, min_stock_alert: packaging.min_stock_alert ?? null,
        cost_per_unit,
      }).select().single();
      if (error) throw error;
      return data;
  ...
  const updatePackaging = useMutation({
    mutationFn: async ({ id, ...updates }: { id: string } & Partial<PackagingFormData>) => {
      ...
      const { data, error } = await supabase.from('packaging').update(updateData).eq('id', id).select().single();
      if (error) throw error;
      return data;
    },
    retry: 1,
  ...
  const deletePackaging = useMutation({
    ...
    retry: 1,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['packaging'] }); toast.success('Embalagem excluída com sucesso!'); },
