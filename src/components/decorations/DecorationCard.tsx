...
import { Copy, Pencil, Trash2, AlertTriangle, RotateCcw } from 'lucide-react';
...
export function DecorationCard({ decoration, onDuplicate, onEdit, onDelete, onReactivate }: DecorationCardProps) {
  ...
  const isInactive = !decoration.is_active;
  ...
  return (
    <div className={`bg-card border border-border rounded-lg p-4 hover:shadow-md transition-shadow ${isInactive ? 'opacity-60' : ''}`}>
      ...
      <div className="space-y-1 text-sm text-muted-foreground mb-3">
        ...
        {decoration.brand && <p>Marca: {decoration.brand}</p>}
        {decoration.supplier && <p>Fornecedor: {decoration.supplier}</p>}
        {decoration.stock_quantity !== null && (() => {
